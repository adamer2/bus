...
manage_voice_list = {'keyboard': [['افزودن ویس ⏬', 'ویس ها 📝'], ['بازگشت 🔙']], 'resize_keyboard': True}
voice_suggestions = {'keyboard': [
    ['پیشنهاد ویس 🔥', 'ویس ها 📝'], ['لغو رای گیری ⏹'], ['بازگشت 🔙']
], 'resize_keyboard': True}
...
manage_playlists = {'keyboard': [
    ['ایجاد پلی لیست 🆕'], ['پلی لیست های من 📝', 'عضویت ها 🔗'], ['بازگشت 🔙']
], 'resize_keyboard': True}
...
manage_playlist_member = {'keyboard': [['ویس ها 📝', 'لغو عضویت ❌'], ['بازگشت 🔙']], 'resize_keyboard': True}
...
public_group = {'inline_keyboard': [[{'text': 'گروه عمومی 👥', 'url': 'https://t.me/PersianMemeChat'}]]}
...
def create_meme_list_keyboard(voices, start: int):
    return [{
...
def add_list_page_keys(keyboard: dict, obj_type: ObjectType, prev_page: int | None, next_page: int | None):
    keyboard['inline_keyboard'].append(list())
    ...
    if next_page is not None:
        keyboard['inline_keyboard'][-1].insert(1, {
            'text': '▶️',
...
@sync_to_async
def make_meme_list(memes, count: int):
    return {'inline_keyboard': [
        create_meme_list_keyboard(
            memes[start:index], start
        ) for index in range(3, count, 3) if memes[(start := index - 3):index]
    ]}


async def make_complete_meme_list(obj_type: ObjectType, memes, prev_page: int, next_page: int):
    temp_keyboard = await make_meme_list(memes, 12)
    add_list_page_keys(temp_keyboard, obj_type, prev_page, next_page)

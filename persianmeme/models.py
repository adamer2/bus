...
from LilSholex.models import BUser
...
class User(BUser):
    class Status(BUser.Status):
        ACTIVE = 'a', 'Active'
        BANNED = 'b', 'Banned'
        FULL_BANNED = 'f', 'Full Banned'

    class Rank(BUser.Rank):
        OWNER = 'o', 'Owner'
        USER = 'u', 'User'
        ADMIN = 'a', 'Admin'
        KHIAR = 'k', 'Khiar'
    ...
    class Menu(BUser.Menu):
        ...
        USER_MANAGE_PLAYLIST_MEMBER = 69, 'User Manage Playlist Member'
    ...
    playlists = models.ManyToManyField('Playlist', 'users', blank=True, through='PlaylistMember')
...
class NewMeme(models.Model):
    ...
    tags = models.CharField(max_length=207, blank=True, null=True)
    usage_count = models.PositiveIntegerField(default=0)
    type = models.PositiveSmallIntegerField(choices=MemeType, default=MemeType.VOICE)
    description = models.CharField(max_length=settings.MAX_DESCRIPTION_LENGTH, blank=True, null=True)
    votes = models.IntegerField(default=0)
    playlist_voice = models.ForeignKey('PlaylistVoice', models.CASCADE, 'memes', blank=True, null=True)
    playlist_member = models.ForeignKey('PlaylistMember', models.CASCADE, 'memes', blank=True, null=True)
    ...
    class Meta:
        ...
        constraints = (
            ...
            models.UniqueConstraint(
                fields=('sender', 'playlist_voice'),
                condition=models.Q(playlist_voice__isnull=False),
                name='unique_playlist_voice_meme'
            ),
            models.UniqueConstraint(
    ...
    def clean(self):
        if not ((self.type == MemeType.VOICE and self.status == NewMeme.Status.ACTIVE
                 and self.visibility == NewMeme.Visibility.PRIVATE
                 and self.playlist_voice_id and self.playlist_member_id) or
                not (self.playlist_voice_id or self.playlist_member_id)):
            raise ValidationError('Both playlist_voice and playlist_member must be specified!')
...
class Meme(NewMeme):
    ...
    async def accept(self):
        from LilSholex.functions import send_message
...
class PlaylistVoice(models.Model):
    ...
    def __str__(self):
        return f'{self.playlist} : {self.voice}'


class PlaylistMember(models.Model):
    playlist = models.ForeignKey('Playlist', models.CASCADE, 'playlist_members')
    user = models.ForeignKey('User', models.CASCADE, 'playlist_members')

    class Meta:
        ordering = ('-id',)
        constraints = (models.UniqueConstraint(fields=('playlist', 'user'), name='unique_playlist_member'),)
...
class PlaylistEvent(models.Model):
    class Type(models.IntegerChoices):
        NEW_MEME = 1, 'New Meme'
        NEW_MEMBER = 2, 'New Member'
    ...
    playlist_voice = models.ForeignKey('PlaylistVoice', models.CASCADE, 'playlist_events', blank=True, null=True)
    playlist_member = models.ForeignKey('PlaylistMember', models.CASCADE, 'playlist_events', blank=True, null=True)

    class Meta:
        ordering = ('id',)
        constraints = (models.CheckConstraint(
            check=models.Q(type=1, playlist_voice__isnull=False, playlist_member__isnull=True) |
            models.Q(type=2, playlist_member__isnull=False, playlist_voice__isnull=True),
            name='valid_playlist_event'
        ),)

    def __str__(self):
        return f'{self.id} : {self.get_type_display()} : {self.get_status_display()}'

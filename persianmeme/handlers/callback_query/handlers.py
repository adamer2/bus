...
from LilSholex.functions import answer_callback_query, edit_message_reply_markup
...
async def handler():
    ...
    match callback_query['data'].split(':'):
        ...
        case (ObjectType.PLAYLIST_MEMBER, playlist_id):
            meme_context.common.PLAYLIST_ID.set(playlist_id)
            await playlist_member.handler()
        case ((
            ...
            if inliner.database.rank != User.Rank.USER:
                try:
                    ...
                    async with TaskGroup() as tg:
                        ...
                        tg.create_task(edit_message_reply_markup(settings.MEME_REPORTS_CHANNEL, processed))
                        tg.create_task(inliner.database.asave())
                    raise RequestInterruption()
                finally:

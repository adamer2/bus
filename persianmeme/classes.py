...
from LilSholex.functions import handle_request_exception, handle_message_params, pin_chat_message
from asgiref.sync import sync_to_async
from django.db import transaction
from LilSholex.context import telegram as telegram_context
from LilSholex.classes import BUser
...
class User(BUser):
    ...
    async def set_database_instance(self):
        ...
        return await super().set_database_instance()
    ...
    async def send_current_meme(self, reply_markup: dict | None = None, reply_to_message_id: int | None = None):
        ...
        async with self.session.get(
            f'{telegram_context.common.BOT_BASE_URL.get()}send{meme_method}',
    ...
    async def get_chat(self):
        async with self.session.get(
            f'{telegram_context.common.BOT_BASE_URL.get()}getChat', params=self._base_params
        ) as response:
    ...
    async def private_voices_count(self):
        return await models.Meme.objects.filter(
            visibility=models.Meme.Visibility.PRIVATE, sender=self.database, playlist_voice__isnull=True
        ).acount()
    ...
    async def delete_owned_meme(self):
        if self.database.current_meme.sender_id == self.database.id and \
            not self.database.current_meme.playlist_voice_id:
            await self.database.current_meme.adelete()
            return True
        return False

    async def _check_meme_existence(self, file_unique_id: str):
        return await models.NewMeme.objects.filter(
    ...
    async def upload_voice(self):
        ...
        if self.database.started and self.database.last_start and \
            ...
            return
        async with self.session.get(
            f'{telegram_context.common.BOT_BASE_URL.get()}sendChatAction',
            params={**self._base_params, 'action': 'upload_voice'}
        ) as response:
    ...
    @sync_to_async
    def get_playlist_members(self):
        return paginate(models.Playlist.objects.filter(playlist_members__user=self.database))

    @sync_to_async
    def get_playlist_member_voices(self):
        return paginate(self.database.sent_memes.filter(playlist_voice__playlist=self.database.current_playlist))

    @sync_to_async
    def get_private_voices(self):
        return paginate(models.Meme.objects.filter(
            sender=self.database, visibility=models.Meme.Visibility.PRIVATE, playlist_voice__isnull=True)
        )
    ...
    def join_playlist(self, playlist_id: str):
        try:
            ...
            return False
        if playlist.creator == self.database:
            return False
        with transaction.atomic():
            self.database.playlist_members.get_or_create(playlist=playlist)
        return playlist
    ...
    def add_voice_to_playlist(self):
        if (first_voice := models.Meme.objects.filter(
            ...
            with transaction.atomic():
                self.database.current_playlist.playlist_voices.get_or_create(voice=first_voice)
            return True
        return False
    ...
    async def send_messages(self):
        if await (messages := models.Message.objects.select_related('sender').filter(
            status=models.Message.Status.PENDING
        )[:settings.MESSAGE_BATCH_SIZE]).aexists():
    ...
    async def process_meme_tags(self):
        ...
        try:
            ...
            if not split_tags or len(split_tags) > settings.MAX_TAG_COUNT:
    ...
    async def validate_meme_description(self):
        if not (text := telegram_context.message.TEXT.get()) or len(text) > 120:
            await self.send_message(
                translations.admin_messages['invalid_meme_description'].format(self.current_meme_translation),
    ...
    async def send_ordered_meme_list(self, ordering: models.User.Ordering):
        ...
        async with TaskGroup() as tg:
            ...
            keyboard_list = tg.create_task(make_meme_list(ordered_memes, 15))
    ...
    async def report_meme(self, meme: models.Meme):
        ...
        if created or not await report.reporters.filter(id=self.database.id).aexists():
            async with TaskGroup() as tg:
                ...
                if created:
                    if message_id := await report.meme.send_meme(
                        ...
                        tg.create_task(pin_chat_message(settings.MEME_REPORTS_CHANNEL, message_id))
